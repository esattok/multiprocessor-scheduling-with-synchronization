...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < numOfQueues; i++) {
        ...
        pthread_cond_init(&((queueTable[i])->cond_v), NULL); // Initialize the condition variable
    }
    ...
    if (useR == 0) {
        while (fscanf(inputFP, "%s", buffer) != EOF) {
            if (!strcmp(buffer, "PL")) {
                ...
                pthread_cond_broadcast(&((queueTable[queueIndex])->cond_v)); // wake up a sleeping thread
    ...
    for (int i = 0; i < numOfQueues; i++) {
        ...
        pthread_cond_broadcast(&((queueTable[i])->cond_v)); // wake up a sleeping thread
    ...
    for (int i = 0; i < numOfQueues; i++) {
        ...
        pthread_cond_destroy(&((queueTable[i])->cond_v)); // Destroy the mutex
        free(queueTable[i]);
    }
    free(queueTable);
...
void* do_task(void* arg_ptr) {
    ...
    while (1) {
        ...
        if (!strcmp(alg, "FCFS")) {
            ...
            // Wait on the condition variable if the queue is empty
            while (queueTable[queueID]->head == NULL) {
                pthread_cond_wait((&(queueTable[queueID]->cond_v)), (&(queueTable[queueID]->mutex)));
            } // We know that there is an item in the queue now

            current = queueTable[queueID]->head;
            
            // End the loop if the dummy item is reached
            if (current->data->pid == -1) {
                /* critical section end */
